...
class Crop(BaseEstimator, TransformerMixin):
    def __init__(self, xmin1, xmax1, ymin1, ymax1, zmin1, zmax1,
                 xmin2=None, xmax2=None, ymin2=None, ymax2=None, zmin2=None, zmax2=None,
                 xmin3=None, xmax3=None, ymin3=None, ymax3=None, zmin3=None, zmax3=None):
    ...
    def transform(self, X, y=None):
        ...
        if self.v3[0] is not None:
            X_crop3 = X[:, self.v3[0]:self.v3[1], self.v3[2]:self.v3[3], self.v3[4]:self.v3[5]]
...
class Histogramize3d(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        if type(X) is np.ndarray:
            ...
            H = np.zeros((X[0].shape[0], len(X), self.n_bins+3), dtype=np.int32)
            for reg in range(len(X)):
                for samp in range(X[reg].shape[0]):
                    H[samp, reg, 0:self.n_bins] = np.histogram(X[reg][samp,:],
                                                                   bins=self.n_bins,
                                                                   range=(self.rmin,
                                                                          self.rmax))[0]
